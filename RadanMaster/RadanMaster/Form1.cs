...
namespace RadanMaster
{
    public partial class Form1 : RibbonForm
    {
        ...
        public Form1()
        {
            ...
            DisplayItems = new BindingList<DisplayItem>();
            DisplayItems = LoadDisplayItems();


            // This line of code is generated by Data Source Configuration Wizard
            //orderItemsBindingSource.DataSource = dbContext.OrderItems.Local.ToBindingList();
            //orderItemsBindingSource.DataSource = DisplayItems.ToList();
        ...
        private BindingList<DisplayItem>LoadDisplayItems()
        {
            BindingList<DisplayItem> displayItems = new BindingList<DisplayItem>();

            foreach (OrderItem item in dbContext.OrderItems.ToList())
            {
                DisplayItem dispItem = new DisplayItem(item);
                    ...
                    if (calcItem.AssociatedNests != null)
                    {
                        foreach (Nest associatedNest in calcItem.AssociatedNests)
                        {
                            if (associatedNest.NestedParts != null)
                            {
                                //*****************************************
                                // not working right.......................
                                // the first block shows  calculates too many, the second one not enough.
                                //*******************************************
                                ...
                                //foreach (NestedParts p in associatedNest.NestedParts)
                                //{
                                //    RadanPart radanPart = rPrj.Parts.Part.Where(prt => prt.ID == p.ID).FirstOrDefault();    // match up the nested part with the radan part

                                //    if (radanPart != null)
                                //    {
                                //        if (p.Part == calcItem.Part && p.ID == radanPart.ID && Path.GetDirectoryName(associatedNest.nestPath) == Path.GetDirectoryName(radanProjectName))
                                //            totalNested += p.Qty;
                                //    }
                                //    else
                                //    {
        ...
        private void gridViewNests_CustomUnboundColumnData(object sender, CustomColumnDataEventArgs e)
        {
            if (e.Column.FieldName == "QtyOnNest")
            {
                ...
                NestedParts part = associatedNest.NestedParts.Where(n => n.OrderItem == selectedItem).FirstOrDefault();
